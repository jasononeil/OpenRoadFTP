...
class hxbase_xml_XmlNode {
	...
	public function addChildAt($child, $index) {
		...
		if($child->testIsDocument()) {
			$indexOffset = null;
			$indexOffset = 0;
			if(null == $child->getter_children()) throw new HException('null iterable');
	...
	public function clearWhitespace() {
		...
		if(null == $this->getter_children()) throw new HException('null iterable');
	...
	public function getChildren($nameFilter, $limit) {
		...
		if($this->testIsElement() || $this->testIsDocument()) {
			if(null == $this->getter_xml()) throw new HException('null iterable');
				...
				if($otherNode->isDescendantOf($this)) {
					...
					if($this->getter_document()->getter_xml() == $otherNode->getter_document()->getter_xml()) {
						...
						if($isOtherNodeDeeperThanThis) {
							...
							$diff = $thisDepth - $otherDepth;
							$thisAncestorOfCommonDepth = $this->getNthParent($diff);
	...
	public function getter_xml() {
		$this->x = hxbase_xml_XmlNode_0($this);
		return $this->x;
	}
	public function getter_fast() {
		$this->f = hxbase_xml_XmlNode_1($this);
		return $this->f;
	}
	public function getter_name() {
		return hxbase_xml_XmlNode_2($this);
	}
	...
	public function getter_numChildren() {
		return Lambda::count($this->getter_xml(), null);
	}
	...
	public function getter_innerText() {
		...
		$textDescendants = $allDescendants->filterByFunction(array(new _hx_lambda(array(&$allDescendants, &$s, &$textDescendants), "hxbase_xml_XmlNode_3"), 'execute'));
		$s1 = new StringBuf();
...
function hxbase_xml_XmlNode_0(&$»this) {
	if($»this->x !== null) {
		return $»this->x;
	} else {
		return Xml::parse("<empty />");
	}
}
function hxbase_xml_XmlNode_1(&$»this) {
	if($»this->f !== null) {
		return $»this->f;
	} else {
		return new haxe_xml_Fast($»this->getter_xml());
	}
}
function hxbase_xml_XmlNode_2(&$»this) {
	if($»this->testIsElement()) {
		return $»this->getter_xml()->getNodeName();
	} else {
		return "#" . Std::string($»this->getter_type());
	}
}
function hxbase_xml_XmlNode_3(&$allDescendants, &$s, &$textDescendants, $x) {
	{
