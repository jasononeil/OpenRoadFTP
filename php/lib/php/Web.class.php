...
class php_Web {
	...
	static function getParamValues($param) {
		...
		$explore = array(new _hx_lambda(array("explore" => &$explore, "param" => &$param, "reg" => &$reg, "res" => &$res), null, array('data'), "{
		...
		$parts = new _hx_array(array_keys($_FILES));
		{
			...
			while($_g < $parts->length) {
				...
				while(!feof($h)) {
					$buf = fread($h, $bsize);
					$size = strlen($buf);
					call_user_func_array($onData, array($buf, 0, $size));
					unset($size,$buf);
				}
				fclose($h);
				unset($tmp,$size,$part,$info,$h,$file,$err,$buf,$bsize);
			}
		}
	}
